...
class L3Dictionary(nn.Module):
    ...
    def forward(self, k_remote: torch.Tensor, v_remote: torch.Tensor):
        ...
        # K3/V3 = codebook 本身 (detach, 使 C 只由 EMA 驱动, 不接 attention 梯度)
        k3 = self.k_up(self.codebook.detach())  # [M, head_dim]
        v3 = self.v_up(self.codebook.detach())  # [M, head_dim]
        ...
        if T_remote > 0:
            ...
            c_n = F.normalize(self.codebook.detach(), dim=-1)  # [M, latent_dim]
            ...
            # commitment loss = ||f - C[idx]||² (梯度只回传到 f)
            c_idx = self.codebook.detach()[hard_idx]  # [B, n_kv, T_remote, latent_dim]
            commitment = ((f - c_idx) ** 2).sum(dim=-1).mean()

            # EMA 更新 codebook (no_grad)
            if self.training and self.ema < 1.0:
                self._ema_update(f.detach(), soft_assign.detach())
        ...
        return k3, v3, commitment

    def _ema_update(self, f: torch.Tensor, soft_assign: torch.Tensor):
        # f: [B, n_kv, T_pool, latent_dim], soft_assign: [B, n_kv, T_pool, M]
        flat_f = f.reshape(-1, self.latent_dim)  # [N, latent_dim]
        flat_a = soft_assign.reshape(-1, self.M)  # [N, M]
        new = torch.matmul(flat_a.t(), flat_f)  # [M, latent_dim]
        count = flat_a.sum(dim=0)  # [M]
...
class PyramisL3DAttention(nn.Module):
    ...
    def forward(
        ...
        return out, past_key_value, balance_loss, sparsity_loss, commitment
...
class PyramisL3DBlock(nn.Module):
    ...
    def forward(
        ...
        attn_out, present_kv, balance_loss, sparsity_loss, commitment_loss = self.self_attn(
        ...
        return hidden_states, present_kv, balance_loss, sparsity_loss, commitment_loss
...
class PyramisL3DModel(PreTrainedModel):
    ...
    def forward(
        ...
        for block in self.blocks:
            hidden_states, present_kv, balance_loss, sparsity_loss, commitment_loss = block(
        ...
        return hidden_states, past_key_values, total_balance, total_sparsity, total_commitment
...
class PyramisL3DForCausalLM(PreTrainedModel):
    ...
    def forward(
        ...
        hidden_states, past_key_values, total_balance, total_sparsity, total_commitment = self.model(
